<template>
  <div class="wrap" v-loading="load">
    <el-form v-show="isShow" :model="forgetForm" :rules="forgetform" ref="forgetForm" class="demo-ruleForm forget-container">
      <!--<h2 class="logo">重置密码</h2>-->
      <div class="logo"><p class="title">众帮互助</p></div>
      <p>修改密码</p>
      <el-form-item prop="phone">
        <el-input type="text" v-model="forgetForm.phone" auto-complete="off" placeholder="手机号" clearable :maxlength="eleven" ></el-input>
      </el-form-item>
      <el-form-item prop="testPass" id="check">
        <el-input type="text" v-model="forgetForm.testPass" auto-complete="off" placeholder="验证码" :maxlength="four"></el-input>
      ...
      <el-form-item prop="pass"  class="demo-input-suffix">
        <el-input :type="passType" v-model="forgetForm.pass" auto-complete="off" :maxlength="six" placeholder="新密码" :input="showIcon()"><i slot="suffix" class="el-input__icon" :class="{'icon-show': passShow,'icon-hide':passHide }" @click.self="showPass(passType)"></i></el-input>
      </el-form-item>
      ...
      <el-form-item style="width:100%;margin-top: 20px" label-width="0">
        <el-button class="reset-btn" :class="{'valid-btn': forgetForm.phone,'':!forgetForm.phone }" @click.native.prevent="resetPass()">确定</el-button>
      </el-form-item>
      ...
      <div class="prompt" v-if="prompt">
        <div class="prompt-box">
          ...
          <div class="prompt-icon"><i :class="{'el-icon-error': fail ,'el-icon-success': success ,'fail': fail ,'success' : success}" ></i></div>
          <p>{{promptMsg}}</p>
        </div>
      </div>
    </el-form>
    <div v-show="!isShow" class="content">
      ...
      <p>修改密码成功,请<el-button type="text" style="color: #F56C6C;" @click="toLogin">重新登录</el-button>!</p>
...
//      输入框控制发送验证码按钮置灰
            ...
            sendCode () {
                ...
                let token = SHA256('zsdfyreuoyamdphhaweyrjbvzkgfdycs').toString()
                let params = 'phone=' + phone + '&token=' + token
                this.$odoo.post('saftyphonecheck', params).then((res) => {
                    if (res.result === '0') {
                        this.load = false
                        this.$message(res.msg)
                    } else if (res.result === '1') {
                        sessionStorage.setItem('account', this.forgetForm.phone)
                        sessionStorage.setItem('secertKey', res.data)
                        this.$odoo.login().then((odooRes) => {
                            if (odooRes.result) {
                                // 登录成功
                                this.load = false
                                this.codeSend()
                                ...
                                this.load = false
                                this.$message('目前仅支持店长重置密码')
                            }
                        }, (odooError) => {
                            this.load = false
                            this.$message('服务器连接失败')
                            console.log(odooError)
                        })
                    } else {
                        this.load = false
                        this.$message(res.msg)
            ...
            // 发送验证码
            codeSend () {
                this.$odoo.sendCode(this.forgetForm.phone).then((res) => {
                    this.testCode = res
                    this.isActive = false
                    let num = 60
                    let that = this
...
<style lang="scss">
  ...
  .wrap{
    ...
    .forget-container {
      ...
      width: 430px;
      padding: 35px;
      background: #fff;margin: 140px auto;
